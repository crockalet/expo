...
export declare class Session extends SharedObject {
    ...
    readonly id: string;
    /**
     * Kind of session. The per-launch session is `'main'`; a session tracking
     * foreground time is `'foreground'`.
     */
    ...
    /**
     * Fetches whether the session is still active. Read lazily so it reflects the
     * session's live state rather than a snapshot from when the object was
     * created.
     */
