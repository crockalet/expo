  ...
  override fun definition() =
    ModuleDefinition {
      ...
      AsyncFunction("addCustomMetricToSession") Coroutine { metric: JsMetric ->
        // The `sessionId` on the incoming metric is ignored: the main session is
        // the only writable session on Android, and `addMetrics` re-stamps the id.
        // Routing through the shared object also gives the FK-safe persist ordering.
        mainSession.addMetrics(listOf(metric.toMetric()))
      }
