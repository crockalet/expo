...
class AppMetricsApplicationLifecycleListeners : ApplicationLifecycleListener {
  override fun onCreate(application: Application) {
    ...
    // during startup would otherwise be unattributable — or missed entirely.
    ProcessIdentity.initialize()
    JvmCrashHandler.install(
      CrashFileWriter.forContext(application).also { it.prepare() }
    ) { ProcessIdentity.sessionId }
