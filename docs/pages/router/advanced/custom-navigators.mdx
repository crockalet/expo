...
`unstable_createStandardRouterNavigator` infers the event map from the component, so you do not pass it again at the call site. Omit the second argument for a navigator that emits no events.
...
> **info** `createProps` receives the raw Expo Router `state` and `dispatch`. These are internal and may have breaking changes between releases, so prefer the `state` and `actions` passed to `NavigatorContent` when they are enough. If something you need to build your navigator is missing from the standard `state`, `actions`, or `emitter`, [let us know in an issue](https://github.com/expo/expo/issues).
