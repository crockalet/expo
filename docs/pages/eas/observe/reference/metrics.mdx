...
Instead of calling `markInteractive()` from an effect, you can render the `<InteractiveMarker />` component at the point your screen becomes interactive — for example, once its initial data has loaded. It calls `markInteractive()` once when it mounts and renders nothing.
